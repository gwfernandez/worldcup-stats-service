...
// ListStadiumsByYear godoc
// @Summary List stadiums used in a championship year with filters and pagination
// @Produce json
// @Param year path int true "Championship Year"
// @Router /api/championships/{year}/stadiums [get]
func (h *ChampionshipHandler) ListStadiumsByYear(c *gin.Context) {
	filter, err := parseChampionshipStadiumFilter(c)
	if err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
		return
	}
...
func parseChampionshipStadiumFilter(c *gin.Context) (domain.ChampionshipStadiumFilter, error) {
	year, err := strconv.Atoi(c.Param("year"))
	if err != nil {
		return domain.ChampionshipStadiumFilter{}, errors.New("invalid year parameter")
	}
	...
	if rawPage := c.Query("page"); rawPage != "" {
		page, err := strconv.Atoi(rawPage)
		if err != nil {
			return domain.ChampionshipStadiumFilter{}, errors.New("invalid page parameter")
		}
		filter.Page = page
	}
...
func parseChampionshipScorerFilter(c *gin.Context) (domain.ChampionshipScorerFilter, error) {
	year, err := strconv.Atoi(c.Param("year"))
	if err != nil {
		return domain.ChampionshipScorerFilter{}, errors.New("invalid year parameter")
	}
	...
	if rawPage := c.Query("page"); rawPage != "" {
		page, err := strconv.Atoi(rawPage)
		if err != nil {
			return domain.ChampionshipScorerFilter{}, errors.New("invalid page parameter")
		}
		filter.Page = page
	}
