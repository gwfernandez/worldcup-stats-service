...
func TestMatchRepository_ListByYear(t *testing.T) {
	t.Run("success with matches", func(t *testing.T) {
		...
		assert.Equal(t, "BRA", result[0].Match.HomeTeam.Code)
		assert.Equal(t, "Brazil", result[0].Match.HomeTeam.Name)
		assert.Equal(t, "POL", result[0].Match.AwayTeam.Code)
		assert.Equal(t, "Poland", result[0].Match.AwayTeam.Name)
