...
func TestGoalRepository_ListByPlayer(t *testing.T) {
	t.Run("success with year filter and nullable fields", func(t *testing.T) {
		...
		rows := mock.NewRows([]string{"year", "match_date", "opponent_team_code", "minute_regular", "penalty", "stage"}).
			AddRow(int32(2018), matchDate, "isl", int32(64), pgtype.Bool{Bool: true, Valid: true}, "group_stage").
			AddRow(int32(2018), pgtype.Date{}, "cro", int32(80), pgtype.Bool{}, nil)
	...
	t.Run("success without year filter", func(t *testing.T) {
		...
		mock.ExpectQuery(`^-- name: ListGoalsByPlayer :many.*`).
			...
			WillReturnRows(mock.NewRows([]string{"year", "match_date", "opponent_team_code", "minute_regular", "penalty", "stage"}))
